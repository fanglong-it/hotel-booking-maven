 ...
 * @author DW
 */
@WebServlet(name = "ViewDetailRoom", urlPatterns = {"/ViewDetailRoom"})
public class ViewDetailRoom extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        String url = "viewRoom.jsp";
        FeedBackDAO feedBackDAO = new FeedBackDAO();
        try {
            String roomNo = request.getParameter("roomNo") == null ? "" : request.getParameter("roomNo");

            RoomDAO roomDAO = new RoomDAO();
            RoomDTO roomDTO = roomDAO.getRoomByRoomNo(Integer.parseInt(roomNo));
            List<FeedBackDTO> feedBackDTOs = feedBackDAO.getListFeedBack(Integer.parseInt(roomNo));
            request.setAttribute("FEED_BACKS", feedBackDTOs);
